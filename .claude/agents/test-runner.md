...
- "Run the [project] test suite: source root `~/openclaw/[project]/src`, suites: [{name: 'unit', command: 'npx ts-node tests/unit.test.ts', runner: 'custom-assert'}], env_vars: {DRY_RUN: '1'}"
- "Run [project] tests with typecheck: source root `~/openclaw/[project]/src`, typecheck: {command: 'npx tsc --noEmit', label: 'TypeScript'}, suites: [{name: 'e2e', command: 'node --test tests/', runner: 'node-test'}]"
