...
11. **Orphaned solutions check:** For each doc in `_system/docs/solutions/`, verify it appears in at least one skill's `required_context` entries (check all `.claude/skills/*/SKILL.md` frontmatter). Solutions docs with no `required_context` linkage from any skill are orphaned — they were captured by compound engineering but have no mechanical read-back path. Flag these for the user with the recommendation to either add a `required_context` entry to the appropriate skill or confirm the doc is discovery-only (no hard linkage needed).
