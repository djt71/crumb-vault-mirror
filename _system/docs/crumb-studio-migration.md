...
For background on the threat model and design rationale, see the colocation spec (retired 2026-06-12; full text in git history at `_system/docs/openclaw-colocation-spec.md`).
...
The plist label is `ai.openclaw.gateway` and the daemon runs as a LaunchDaemon at `/Library/LaunchDaemons/ai.openclaw.gateway.plist`. These are used in the kill-switch runbook in the colocation spec (retired; git history at `_system/docs/openclaw-colocation-spec.md`).

#### Step 12: Connect messaging platforms

Connect platforms using **burner accounts** (not your personal accounts). See the colocation spec (retired; git history at `_system/docs/openclaw-colocation-spec.md`) for per-platform kill-switch procedures.
