...
|-------|----------|-------------|-------------|----------------|-------------------|
| action-architect | `.claude/skills/action-architect/SKILL.md` | ~1,435 | Skill trigger phrase | Workflow decomposition changes | [[02-building-blocks]] §Skills |
| attention-manager | `.claude/skills/attention-manager/SKILL.md` | ~2,343 | "plan my day", "daily attention" | Goal-tracker schema, attention planning | [[03-runtime-views]] §AKM |
| audit | `.claude/skills/audit/SKILL.md` | ~1,814 | "audit vault", "check for drift" | Vault-check rule changes | [[05-cross-cutting-concepts]] §vault-check |
| checkpoint | `.claude/skills/checkpoint/SKILL.md` | ~545 | Phase transitions | Context management changes | [[05-cross-cutting-concepts]] §Context Budget |
| code-review | `.claude/skills/code-review/SKILL.md` | ~3,016 | "review this code" | Review panel model changes | [[02-building-blocks]] §Code Review |
| deck-intel | `.claude/skills/deck-intel/SKILL.md` | ~2,621 | "process this deck" | Extraction format changes | [[02-building-blocks]] §Skills |
| diagram-capture | `.claude/skills/diagram-capture/SKILL.md` | ~2,347 | "capture this diagram" | Image classification changes | [[02-building-blocks]] §Skills |
| excalidraw | `.claude/skills/excalidraw/SKILL.md` | ~3,375 (+3,015 supplements) | "excalidraw", "sketch this" | Excalidraw format changes | [[02-building-blocks]] §Skills |
| feed-pipeline | `.claude/skills/feed-pipeline/SKILL.md` | ~3,377 | "process feed items" | FIF schema changes, tier logic | [[03-runtime-views]] §Feed Pipeline |
| inbox-processor | `.claude/skills/inbox-processor/SKILL.md` | ~6,705 | "process inbox" | Sentinel format, routing rules | [[02-building-blocks]] §Skills |
| learning-plan | `.claude/skills/learning-plan/SKILL.md` | ~2,881 | "learn", "training plan" | Pedagogy framework changes | [[02-building-blocks]] §Skills |
| lucidchart | `.claude/skills/lucidchart/SKILL.md` | ~5,379 (+1,713 supplement) | "lucidchart" | Lucidchart API changes | [[02-building-blocks]] §Skills |
| meme-creator | `.claude/skills/meme-creator/SKILL.md` | ~1,169 | "make a meme" | TMDB API changes | [[02-building-blocks]] §Skills |
| mermaid | `.claude/skills/mermaid/SKILL.md` | ~4,012 | "diagram this", "chart" | Mermaid syntax changes | [[02-building-blocks]] §Skills |
| obsidian-cli | `.claude/skills/obsidian-cli/SKILL.md` | ~875 | Indexed vault queries | Obsidian CLI version changes | [[02-building-blocks]] §Skills |
| peer-review | `.claude/skills/peer-review/SKILL.md` | ~2,634 | "peer review", "get review" | Review panel model changes | [[02-building-blocks]] §Skills |
| researcher | `.claude/skills/researcher/SKILL.md` | ~4,136 | "research", "investigate" | Pipeline stage changes | [[02-building-blocks]] §Skills |
| startup | `.claude/skills/startup/SKILL.md` | ~402 | Session start | Startup sequence changes | [[03-runtime-views]] §Session Lifecycle |
| sync | `.claude/skills/sync/SKILL.md` | ~480 | Session end, milestones | Commit/push logic changes | [[05-cross-cutting-concepts]] §Git Patterns |
| systems-analyst | `.claude/skills/systems-analyst/SKILL.md` | ~1,643 | "analyze this", "write a spec" | Spec schema changes | [[02-building-blocks]] §Skills |
| vault-query | `.claude/skills/vault-query/SKILL.md` | ~704 | "query the vault" | Query format changes | [[02-building-blocks]] §Skills |
| writing-coach | `.claude/skills/writing-coach/SKILL.md` | ~1,005 | "improve this", "review my writing" | Writing evaluation changes | [[02-building-blocks]] §Skills |

**Skill layer total:** ~52,897 tokens (all 22 skills; only 1-3 loaded per session)
...
|-------|----------|-------------|-------------|----------------|-------------------|
| code-review-dispatch | `.claude/agents/code-review-dispatch.md` | ~2,074 | code-review skill dispatch | Model/reviewer changes | [[02-building-blocks]] §Subagents |
| peer-review-dispatch | `.claude/agents/peer-review-dispatch.md` | ~2,656 | peer-review skill dispatch | Model/reviewer changes | [[02-building-blocks]] §Subagents |
| test-runner | `.claude/agents/test-runner.md` | ~1,083 | Test execution | Test framework changes | [[02-building-blocks]] §Subagents |
...
| Skills (all 22) | 22 (+4 supplements) | ~58,626 | 1-3 per session |
...
| Subagents | 3 | ~5,812 | On dispatch |
| System config | 4 | ~4,643 | Per hook/skill |
| **Total inventory** | **53** | **~94,565** | — |
