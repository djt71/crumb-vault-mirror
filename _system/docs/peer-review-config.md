...
models:
  ...
  deepseek:
    ...
    curl_timeout: 240  # per-provider override (A10, 2026-08-14): deepseek-reasoner timed out at the global 120s in the 2026-07-07 akm-refresh panel round
  grok:
    model: grok-4.3
...
*Watch tally — review 3 (2026-07-07, akm-refresh specification): 8 findings — 0 fabrications, 1 incorrect + severity-inflated (GRK-F2: CRITICAL "internal inconsistency" that dissolves on the source evidence; artifact under-explanation contributed), 1 noise (GRK-F8: dependency edit based on partial read of the task). Fast/short again (16.5s, 962 tokens). Landed: GRK-F4 (consensus), GRK-F6 (unique — hook/CLAUDE.md overlap window, genuinely useful), GRK-F1 (consensus UVC sweep). **WATCH CLOSED 2026-07-07 — verdict: keep in `default_reviewers`.** Three-review pattern: zero fabrication, ~1 misread + ~1 noise per round, consistently 1–2 real unique findings at the panel's lowest cost and latency. Residual handling: treat GRK CRITICALs with suspicion (2 of 3 reviews inflated severity on its lead finding); weight its unique MINORs seriously.*
