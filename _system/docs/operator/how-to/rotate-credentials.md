...
| Telegram bot token(s) | Was: LaunchAgent plist env vars | Was: `ai.openclaw.awareness-check`, `health-ping`, `vault-health`, `backup-status` (Tess messaging) | None of the current `com.crumb.*` plists (checked all 11 directly) carry a Telegram token in `EnvironmentVariables`. The messaging layer these tokens served is gone. Revocation candidate — verify with the bot provider (@BotFather) whether the token is still even active before spending time revoking. |
| X (Twitter) OAuth | Keychain (dynamic, auto-refresh) | feed-intel framework | feed-intel-framework is dead per agentic-sunset (2026-06-11/12). No live consumer. Revocation candidate. |
| Mistral API key | `~/.config/crumb/.env` (key name confirmed present) | Not found in the 2026-04-11 doc; grep shows only `Projects/tess-v2/` and `Projects/mission-control/` design-doc mentions, both non-code | No script or skill consumer found. Likely dead-project residue (mission-control is `status: paused`, tess-v2 is DONE). Revocation candidate — or confirm with operator whether this was meant for a not-yet-implemented feature. |
| Lucidchart API key (`LUCID_API_KEY`) | `~/.config/crumb/.env` (key name confirmed present) | Was: `lucidchart` skill | No `lucidchart` skill exists in `.claude/skills/` (current skill roster has `mermaid`, which per its description is "the default for all diagram/chart requests" and also covers Excalidraw). `setup-crumb.sh` still checks for the key's presence as an optional pass, but nothing consumes it. Revocation candidate, or restore the skill if diagramming needs still call for the Lucid API specifically. |
...
- [ ] For plist-embedded credentials (e.g., `com.crumb.dashboard`'s `HEALTHCHECKS_API_KEY`): confirm it's still appropriate to store in the plist directly, or migrate to Keychain/env per the Keychain-not-env convention
