...
|-----------|---------|-----------------|--------|
| OpenRouter API key | ~~`~/.config/crumb/.env`~~ **Correction 2026-07-14 (AS-032):** actual location was `/Users/openclaw/.openclaw/openclaw.json` (`models.providers.openrouter.apiKey`); no `OPENROUTER` line exists in `.env` | Tess Voice cloud inference (Kimi K2.5 primary, Qwen 3.6 failover) | Tess Voice / tess-v2 runtime is fully decommissioned. **APPROVED FOR REVOCATION 2026-07-14 (AS-032)** — **REVOKED at OpenRouter dashboard 2026-08-10** (operator-confirmed); local copy deleted with openclaw.json same date. |
| OpenClaw gateway credential | `/Users/openclaw/.openclaw/openclaw.json` (`gateway.auth` — mode `password`, not a token) | OpenClaw gateway auth | `ai.openclaw.gateway` LaunchDaemon confirmed absent. **AS-032 decision 2026-07-14: delete `openclaw.json` + all 11 `.bak*`/`.pre-*` copies** (file was a residual secrets cache: OpenRouter, Perplexity, Discord bot tokens ×2, Telegram, stale OpenAI/Anthropic copies). Deletion requires sudo — operator-executed. **DELETED 2026-08-10: all 12 files removed, verified gone (Claude glob check same session).** The gateway password also appeared in session output 2026-07-14; do not reuse it. `openclaw` account removal remains out of scope. |
| Telegram bot token(s) | Was: LaunchAgent plist env vars | Was: `ai.openclaw.awareness-check`, `health-ping`, `vault-health`, `backup-status` (Tess messaging) | None of the current `com.crumb.*` plists (checked all 11 directly) carry a Telegram token in `EnvironmentVariables`. The messaging layer these tokens served is gone. **Operator decision 2026-07-06: keep dormant** (no BotFather revocation). Consumer check completed same day — `vault-health.sh` and `backup-status.sh` scripts (not just plists) verified zero Telegram references. **2026-07-14 (AS-032): local copies removed** with the Keychain namespace scrub and openclaw.json deletion — dormant-at-provider stands; regenerate via BotFather if ever needed. |
| X (Twitter) OAuth | ~~Keychain~~ **Local copies DELETED 2026-07-14 (AS-032):** the entire `x-feed-intel.*` login-Keychain namespace (9 entries: x-access/refresh-token, x-client-id/secret, twitterapi-io-key, youtube-api-key, telegram-bot-token/chat-id, anthropic-api-key stale copy) removed | feed-intel framework | **REVOKED at X developer portal 2026-08-10** (operator-confirmed). |
| Perplexity API key | Was: `openclaw.json` (`tools.web.search.perplexity.apiKey`) — found by AS-032 sweep, absent from prior doc | Tess/OpenClaw web search | **APPROVED FOR REVOCATION 2026-07-14** — **REVOKED at Perplexity dashboard 2026-08-10** (operator-confirmed); local copy deleted with openclaw.json same date. |
| Discord bot tokens ×2 (`tess-discord`, `mechanic-discord`) | Was: `openclaw.json` (`channels.discord.accounts.*.token`) — found by AS-032 sweep, absent from prior doc | Tess Ops Discord server bots | **APPROVED FOR REVOCATION 2026-07-14** — **EXECUTED 2026-08-10** (operator-confirmed): bot applications deleted at the Discord developer portal, 'Tess Ops' server deleted; local copies deleted with openclaw.json same date. |
| twitterapi.io key | Was: Keychain `x-feed-intel.twitterapi-io-key` — **local copy deleted 2026-07-14** | feed-intel framework | **APPROVED FOR REVOCATION 2026-07-14** — **REVOKED at twitterapi.io dashboard 2026-08-10** (operator-confirmed). |
| YouTube API key | Was: Keychain `x-feed-intel.youtube-api-key` — **local copy deleted 2026-07-14** | feed-intel framework | **APPROVED FOR REVOCATION 2026-07-14** — **DELETED in Google Cloud console 2026-08-10** (operator-confirmed). |
| Mistral API key | `~/.config/crumb/.env` (key name confirmed present) | Not found in the 2026-04-11 doc; grep shows only `Archived/Projects/tess-v2/` and `Projects/mission-control/` design-doc mentions, both non-code | No script or skill consumer found. Likely dead-project residue (mission-control is `status: paused`, tess-v2 is DONE). **REVOKED 2026-07-06** — operator revoked at the Mistral console; `.env` line removed same day (dated backup deleted after revocation). |
