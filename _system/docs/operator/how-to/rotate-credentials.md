...
|-----------|---------|-----------------|--------|
| OpenRouter API key | ~~`~/.config/crumb/.env`~~ **Correction 2026-07-14 (AS-032):** actual location was `/Users/openclaw/.openclaw/openclaw.json` (`models.providers.openrouter.apiKey`); no `OPENROUTER` line exists in `.env` | Tess Voice cloud inference (Kimi K2.5 primary, Qwen 3.6 failover) | Tess Voice / tess-v2 runtime is fully decommissioned. **APPROVED FOR REVOCATION 2026-07-14 (AS-032)** — operator revokes at OpenRouter dashboard; local copy removed with the openclaw.json deletion (below). |
| OpenClaw gateway credential | `/Users/openclaw/.openclaw/openclaw.json` (`gateway.auth` — mode `password`, not a token) | OpenClaw gateway auth | `ai.openclaw.gateway` LaunchDaemon confirmed absent. **AS-032 decision 2026-07-14: delete `openclaw.json` + all 11 `.bak*`/`.pre-*` copies** (file was a residual secrets cache: OpenRouter, Perplexity, Discord bot tokens ×2, Telegram, stale OpenAI/Anthropic copies). Deletion requires sudo — operator-executed. The gateway password also appeared in session output 2026-07-14; do not reuse it. `openclaw` account removal remains out of scope. |
...
| X (Twitter) OAuth | ~~Keychain~~ **Local copies DELETED 2026-07-14 (AS-032):** the entire `x-feed-intel.*` login-Keychain namespace (9 entries: x-access/refresh-token, x-client-id/secret, twitterapi-io-key, youtube-api-key, telegram-bot-token/chat-id, anthropic-api-key stale copy) removed | feed-intel framework | **Provider-side revocation still pending — operator action at X developer portal.** |
| Perplexity API key | Was: `openclaw.json` (`tools.web.search.perplexity.apiKey`) — found by AS-032 sweep, absent from prior doc | Tess/OpenClaw web search | **APPROVED FOR REVOCATION 2026-07-14** — operator revokes at Perplexity dashboard; local copy dies with openclaw.json deletion. |
| Discord bot tokens ×2 (`tess-discord`, `mechanic-discord`) | Was: `openclaw.json` (`channels.discord.accounts.*.token`) — found by AS-032 sweep, absent from prior doc | Tess Ops Discord server bots | **APPROVED FOR REVOCATION 2026-07-14** — operator deletes the bot applications at the Discord developer portal alongside the 'Tess Ops' server deletion; local copies die with openclaw.json deletion. |
| twitterapi.io key | Was: Keychain `x-feed-intel.twitterapi-io-key` — **local copy deleted 2026-07-14** | feed-intel framework | **APPROVED FOR REVOCATION 2026-07-14** — operator revokes at twitterapi.io dashboard. |
| YouTube API key | Was: Keychain `x-feed-intel.youtube-api-key` — **local copy deleted 2026-07-14** | feed-intel framework | **APPROVED FOR REVOCATION 2026-07-14** — operator deletes the key in Google Cloud console. |
