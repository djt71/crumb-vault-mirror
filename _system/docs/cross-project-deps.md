...
| XD-028 | CLAUDE.md signal-scan paragraph deletion (ownership: akm-refresh — transferred from VO-037) | new-content hook live + observed firing (≥3 confirmed organic fires, or operator decision if fewer by soak close) | akm-refresh | AKM-008 (trigger) / AKM-010 (window close) | pending | Added at akm-refresh SPECIFY (2026-07-07); rewritten 2026-08-11 at plan approval. Original mechanism assumed VO-037 would execute the deletion, but vault-optimization closed DONE 2026-08-10 with the paragraph intact (correctly honoring this dependency) — deletion ownership orphaned, transferred to akm-refresh. AKM-008 go-live records the deletion trigger; AKM-010 closes the overlap window (delete on trigger met, else operator disposition). Overlap window (peer review GRK-F6): between R4 hook go-live and deletion, both hook and behavioral instruction are live — double-surfacing possible; accepted, now bounded by AKM-010 closure instead of VO-037 execution. |
