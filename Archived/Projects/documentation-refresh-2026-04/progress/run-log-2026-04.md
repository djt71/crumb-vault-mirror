...
1. **`ai.openclaw.*` (legacy Tess operations, pre-TV2):**
   - `ai.openclaw.bridge.watcher` — running (pid 745), Python KeepAlive, `/Users/danny/Library/LaunchAgents/ai.openclaw.bridge.watcher.plist`
...
**Bridge-watcher service shape:** Unchanged since original architecture doc. Python-based, KeepAlive, path `/Users/danny/Library/LaunchAgents/ai.openclaw.bridge.watcher.plist`. The `ai.openclaw.*` namespace here is legacy — functions have been duplicated into `com.tess.v2.*` for most services, but bridge-watcher remains on the ai.openclaw namespace.
