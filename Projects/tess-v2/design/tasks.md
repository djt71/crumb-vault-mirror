...
| TV2-057a | State-machine semantic fix (Class C) | done | TV2-038 | medium | code | Per `tv2-057-promotion-integration-note.md` §4, §6. Code landed in commits `bd0482a` (classifier + COMPLETED enum + downgrade + tests, 455 pass) and `037c363` (backfill script + runbook). **Backfill executed 2026-04-18** after TV2-038 Phase 5 close (verdict PASS, see `tv2-038-validation-report.md` §5.2). Live run migrated 5836 rows from `outcome='staged'` → `outcome='completed'` across 14 Class C services (daily-attention untouched as only remaining Class A post-TV2-057b). Sentinel `dead_letter_reason='tv2-057a-backfill'` applied to all affected rows (reversible per runbook §4). Before/after: staged 6637 → 802, completed 1169 → 7012. Pre-backfill snapshot at `~/.tess/state/run-history.db.pre-tv2-057a-backfill`. Runbook: `Projects/tess-v2/design/tv2-057a-backfill-runbook.md` (status: released). |
| TV2-057b | Schema + Amendment landing | done | TV2-057a | medium | design+code | Landed 2026-04-17. §2.2 resolved C2 (generation-time bake-in). `canonical_outputs` structured field added to `service-interfaces.md` + contract schema v1.1.0 → v1.2.0 in `src/tess/contract.py` with 13 validation rules. §3.4.2 dispatch-modes Amendment added to `staging-promotion-design.md`; §13 Q1 closed. §4.4 audit surfaced three tensions (operator decisions): vault-health ownership deferred to TV2-040; connections-brainstorm reclassified A→C per integration-note §5; daily-attention direct-write landmine confirmed → TV2-057d migration spec drafted (`tv2-057d-daily-attention-migration.md`). Amendment AB spec doc: `spec-amendment-AB-canonical-outputs.md`. Classifier primary predicate swapped to `canonical_outputs`-check with transitional allowlist fallback. Tests: 475/475 pass. Commits: `f336ae9` (code) + `461ffb6` (AB tagging). |
| TV2-057c | Write-lock acquisition at run-entry (Class A only) | done | TV2-057b | medium | code | Landed 2026-04-18. Wired `WriteLockTable.acquire_locks()` into `_cmd_run` before Ralph loop, Class A only (classifier via `is_side_effect_contract`). `BEGIN IMMEDIATE` all-or-nothing already present in `locks.py`. **R2 chosen** (exit-and-wait-for-next-cadence): on lock-denied, return exit code **75** (`EX_TEMPFAIL`), no `run_history` row, quiet stderr diagnostic. Locks released via `try/finally` on every terminal path (STAGED, COMPLETED, ESCALATED, DEAD_LETTER, and on exceptions). Release failure logged to stderr but does NOT affect return code (zombie lock manifests as a deny on next invocation — visible via existing mechanisms). New module `src/tess/lock_deny.py` tracks consecutive denies per service with atomic-write JSON at `~/.tess/state/lock-denies.json`; at `CONSECUTIVE_DENY_THRESHOLD=5` writes a Z4 candidate marker to `~/.tess/state/z4-candidates/{service}.json`. Startup hook (`_system/scripts/session-startup.sh`) reads the candidates dir and surfaces count + service list in the startup summary so operators see persistent contention before Z Phase B lands. Known-acceptable race: concurrent `_cmd_run` invocations racing on deny may undercount (both read 4, both write 5); worst case Z4 marker fires one cadence later than ideal. Not worth file-locking. `CanonicalOutput.resolve(now: datetime)` added with explicit-now parameter — same `now` flows through to promotion resolution in TV2-057d so placeholders stay stable. Tests: 34 new (lock_deny module: 14; resolver: 6; CLI lock wiring: 14 — Class A acquire + release on all 4 terminal outcomes including exceptions, Class C bypass, lock-denied exit 75 + no history row + stderr + counter increment, successful-acquire counter reset, release-failure logged-but-ignored, concurrent contention integration test with real `WriteLockTable`). Full suite 509/509 pass. Commits: pending commit this session. |
...
| TV2-039 | Production cutover decision | done | TV2-038 | high | decision | Decision document. All services pass validation. Rollback procedure documented and tested. Danny approves. Cost under $50/month target. |
