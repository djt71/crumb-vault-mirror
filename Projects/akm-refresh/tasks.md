...
| AKM-004 | Fixture v2 re-baseline + soak definition and start | pending | AKM-003 | low | software | Fixture `version: 2` baseline recorded? Soak checklist in run-log with start date? Empty-brief-rate and noise-flag conventions (success criterion 7) documented? Soak metrics scoped to the three pre-existing triggers (new-content-hook events logged but excluded from criterion-7 evaluation — amendment 2026-08-11)? |
...
| AKM-010 | Execute and close the ≥2-week soak | pending | AKM-004 | low | software | Soak window ≥14 days from AKM-004 start date completed? Run-log verdict written (pass, or failures enumerated with disposition per criterion-7 fail rules)? Success criteria 1–7 each explicitly evaluated in the closure entry? XD-028 overlap window closed — CLAUDE.md paragraph deleted if the AKM-008 trigger fired, or operator disposition recorded (amendment 2026-08-11)? |
