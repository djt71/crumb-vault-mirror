...
| AS-011 | Pause healthchecks.io check `tess-mac-studio-health` (operator: UI pause, or provide full-access API key — local key is read-only) | blocked | AS-010 | medium | software | Check status = paused, verified; zero "down" alerts received: YES/NO — **blocked on operator** |
