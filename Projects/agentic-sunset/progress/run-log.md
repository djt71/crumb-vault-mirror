...
**Diagnosis:** The automation was `tess-operations` **TOP-017** (archived project), on the personal account **dturner71@gmail.com** (not the Tess-era "workspace" address — a foreign-domain identity of unknown origin, scrubbed from configs 2026-07-07): 16 Gmail labels (`@Agent/* @Trust/* @Risk/* @Action/* P/*`) + **3 server-side Gmail filters** (`gmail.settings.basic` scope). The damage was **Filter A** — newsletters, criteria `unsubscribe OR "view in browser" OR "manage preferences"` → apply `@Agent/IN` + `@Trust/External` + **skip inbox**. The TOP-031 triage *script* that was meant to consume `@Agent/IN` died in the AS teardown, but **the filters live server-side at Google and kept running ~3 months** — force-archiving incoming mail with nothing processing it. The criteria also swept the operator's **Beacon Zen sangha Google Group** (its list footer carries an unsubscribe link → read as "newsletter") and transactional mail (a BMW open safety-recall), so genuinely-wanted mail was buried, not just promos. The 16 `@Agent` labels were already gone, so swept mail had **no label** — findable only in All Mail. Exactly the "mail went missing" failure `tess-google-services-spec §3.2` warned about.
...
- **Day 4 — 2026-07-04 ✅ GREEN** (live check, session opener). Backup: `crumb-vault-2026-07-04_0300.tar.gz` (71 MB, `gzip -t` OK), `backup-status.json` status ok / ageHours 7. *Size drop from ~121 MB is expected — first tarball after VO-028 B1 deleted `Archived/` (~149M) + VO-029 deleted the tess-v2 venv on 07-03; not a backup fault.* drive-sync: 05:00 run DONE, NotebookLM + Computer sync clean (plus three clean post-commit runs evening of 07-03). Alerts: none (healthchecks removed). vault-web :8843 → 200; keep-set = 10 `com.crumb.*` listed, cloudflared PID 20409 + vault-web PID 20412 continuous since the 07-01 restore ⇒ no reboot. Tree: clean. `/dev/console` owner = danny (GUI session up).
- **Day 5 — 2026-07-05 ✅ GREEN** (live check, session opener). Backup: `crumb-vault-2026-07-05_0300.tar.gz` (72 MB, `gzip -t` OK), `backup-status.json` status ok / ageHours 11, marker agrees. drive-sync: 05:00 run DONE, NotebookLM + Computer sync clean (plus clean runs midday 07-04). Alerts: none (healthchecks removed). vault-web :8843 → 200; keep-set = 10 `com.crumb.*` listed, cloudflared PID 20409 + vault-web PID 20412 continuous since the 07-01 restore ⇒ no reboot. Tree: clean. `/dev/console` owner = danny (GUI session up).
- **Day 6 — 2026-07-06 ✅ GREEN** (live check, session opener). Backup: `crumb-vault-2026-07-06_0300.tar.gz` (74 MB, `gzip -t` OK), `backup-status.json` status ok / ageHours 8, marker agrees. drive-sync: 05:00 run DONE, NotebookLM + Computer sync clean (plus three clean post-commit runs evening of 07-05). Alerts: none (healthchecks removed). vault-web :8843 → 200; keep-set = 10 `com.crumb.*` listed, cloudflared PID 20409 + vault-web PID 20412 continuous since the 07-01 restore ⇒ no reboot. Tree: clean. `/dev/console` owner = danny (GUI session up).
- **Day 7 — 2026-07-07 ✅ GREEN** (backfilled 2026-07-13 — sessions ran 07-07 but none ran the AS opener; evidence per documented backfill method + Drive-side corroboration). Backup: `crumb-vault-2026-07-07_0300.tar.gz` (75 MB) present in iCloud backups dir. drive-sync: post-commit runs fired after the 22:11 (`566b1f70`) and 23:06 (`42c34e7f`) commits — Drive-side mtimes on `gdrive:crumb-docs/architecture/*` = 2026-07-07 22:11 / 23:07 (local `/tmp` logs lost to the 07-13 reboot; Drive mtimes are the surviving evidence). Alerts: none (healthchecks removed). vault-web/keep-set: cloudflared PID 20409 + vault-web PID 20412 continuous from the 07-01 restore until the clean shutdown 2026-07-13 15:00 ⇒ no reboot in window, stack up all day. Tree: clean at the 23:06 commit (pre-commit vault-check passed). **7/7 CONSECUTIVE GREEN → AS-031 COMPLETE.**
...
## 2026-07-13 — AS-031 soak v2 COMPLETE (Day 7 backfilled GREEN, 7/7) → AS-032 opens

**Trigger:** operator soak-status opener. Day 7 (2026-07-07) was never ticked — sessions ran that day (skills-library §C execution, architecture refresh, vault-check §32) but none ran the AS opener, and no sessions at all 07-08→07-12. Backfilled per the documented method (`backup-status.json` + tarball chain) plus Drive-side corroboration — full evidence in the tracker's Day 7 line. **AS-031 DONE.**

**Beyond-window evidence (07-08→07-13, informational):** unbroken daily tarball chain `crumb-vault-2026-07-08_0300` → `…07-13_0300` (~75–76 MB each); `backup-status.json` at check time: latestFile 07-13_0300, ageHours 16, status ok; latest tarball `gzip -t` OK. The stack ran uninterrupted for six further days past the soak window.

**Reboot note (post-window, 2026-07-13):** clean shutdown 15:00 → reboot 15:05 → operator GUI-logged-in per the documented operating rule (service-inventory "Resurrection is GUI-login-gated"). Verified post-login: keep-set 10/10 loaded in `gui/503`, cloudflared PID 1468 + vault-web PID 1470 live, :8843 → 200, `/dev/console` owner = danny. **First real-world exercise of the operating assumption since it was documented — it held.** Expected side effect: `/tmp` logs (drive-sync, vault-backup stdout) cleared by the reboot; drive-sync next fires at 05:00 or on post-commit.

**Tree note (not a soak item — post-window):** 8 unstaged deletions in `Projects/vault-optimization/reviews/raw/` (2026-06-10 peer-review raw JSONs) are `vault-gc` 30-day-TTL retention prunes — routine GC churn, to be committed as churn per precedent (`5e80c8fd` class). Logged in the VO run-log.

**State:** AS-031 done. **AS-032 is the sole remaining task** — external-artifact sweep (Google Calendar 'Agent — Staging'/'Agent — Followups', Drive `00_System/Agent/*`, 'Tess Ops' Discord; OpenRouter/OpenClaw-gateway-token/X-OAuth revocation decisions), final compound routing, archival proposals (incl. tess-v2 KB-exception question). Sweep inputs at `design/external-artifacts/`. Not started this session — awaiting operator scheduling.

## 2026-07-13 — Session-end (compound evaluation)

**Session summary:** Dual soak-status check (operator opener). AS-031: Day 7 backfilled GREEN → 7/7 → **AS-031 COMPLETE**, AS-032 now sole remaining task (entry above). VO M5: WS4 backfilled (07-07), WS5 = today, soak now WS-bound (VO run-log). Bookkeeping updated across both projects: run-logs, project-state ×2, claude-ai-context.md refreshed (was 7 days stale, flagged at startup). Plus: vault-gc's 8 `reviews/raw/` TTL prunes identified and committed as churn.

**Compound evaluation:** Two data points, no promotion:
1. **Evidence durability for backfills:** `/tmp` logs (drive-sync, vault-backup stdout) do not survive reboots — the Day 7 backfill succeeded only because persistent markers (`backup-status.json`, tarball chain) and **remote-side state (Drive rclone mtimes)** carried the evidence. When a check depends on log history, prefer persistent/remote artifacts over `/tmp`. Single occurrence with an existing mitigation already in the documented backfill method — noted here, not promoted.
2. **Session-opener checks skip silently when sessions don't open with them:** Day 7 was missed by three sessions that ran on 07-07 without the AS opener — same mechanism class as the v1 soak failure (accepted limitation, operator-documented). Recurrence data point for the known pattern; no action (soak is now complete, mechanism retires with it).

**Protocol steps (7-step):** (1) log — this entry + VO close note. (2) project-state refresh — both projects ✓ (AS: active_task AS-032; VO: WS tracker + WS-bound note). (3) failure-log — not warranted (clean session, zero rework beyond one self-caught duplicate YAML key). (4) code review sweep — N/A (no repo_path). (5) build verification — N/A. (6) qmd update — at close. (7) commit & push — delta = logs/state/context doc + the 8 GC deletions (churn class, flagged to operator in-session); descriptive commit.
