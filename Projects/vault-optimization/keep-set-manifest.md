...
writing-coach→peer-review (#12), checkpoint→audit (#13) — list fully
dispositioned at B5 pack approval 2026-06-10: #11/#13 execute, #8/#12
declined by operator (keep standalone). Dispositions marked `prop:` are
proposals — frozen at VO-023 (B5 pack) after VO-017/019/020.
...
| critic | skill | proven-active (light) | registered at vault audit session-log.md:289; 14 commits latest 2026-04-11; db 0; deferred merge declined by operator at B5 pack approval | **keep** (operator decision 2026-06-10 — merge rejected, stays standalone) | joint (B5; AS M6 gate) | — |
...
| diagram-capture | skill | proven-active (dormant ~3mo) | VO-019 composition check: deck-intel is the only live composer (SKILL.md:88 call); inbox-processor names it in description only; standalone use dormant | prop: merge-into:deck-intel (VO-023 #F6 — SkillsBench merges-beat-borderline-keeps) | joint (B5; AS M6 gate) | — |
| feed-pipeline | skill | superseded (partial) | consumes `_openclaw/inbox/` + FIF SQLite — FIF runtime decommissioned 2026-05-28, agentic layer 2026-06-10 (AS run-log); intake dir deliberately stays open (operator decision); 14 commits latest 2026-03-13; db 0 | prop: keep-with-strip (VO-023 #F7 — strip FIF-SQLite + dashboard-promotion stages, keep inbox classification/promotion core; pends AS-028 concurrence) | joint (B5; AS M6 + AS-028) | — |
...
| peer-review | skill | proven-active | dispatched 2026-06-10 ×2 (spec + action-plan reviews, VO run-log); 47 commits; peer-review-config.md roster | keep (critic/writing-coach merges declined by operator — no longer merge target) | joint (B5; AS M6 gate) | — |
...
| vault-query | skill | structural-necessity (partial supersession) | absorbed obsidian-cli CLI patterns 2026-03 (session-log.md:148); CLAUDE.md File Access still cites "obsidian-cli skill" (stale name — B6 fix); Tess-dispatch half dark since agentic-sunset; 3 commits | prop: keep-with-rewrite (VO-023 #F7 — strip Tess-dispatch/brief surface, keep CLI patterns + retrieval; pends AS-028 concurrence) | joint (B5; AS M6 + AS-028) | — |
| writing-coach | skill | proven-active (light) | required_context linkage session-log-2026-02.md:128; skill reformat :306; 4 commits latest 2026-04-21; deferred merge declined by operator at B5 pack approval | **keep** (operator decision 2026-06-10 — merge rejected, stays standalone) | joint (B5; AS M6 gate) | — |
...
| test-runner | agent | contingency-keep | no parent skill — spawned by main session for repo-backed projects; mission-control (repo_path) active in TASK phase | **keep** (VO-023 — re-check at VO-035 close-out) | joint (B5; AS M6 gate) | — |
...
| glean-prompt-engineer | overlay | proven-active (light) | 1 log file; career/Infoblox enterprise-search lens; no structural skill refs | keep (VO-023 re-check confirmed — operator work surface, cost = one index row) | VO (B4) | — |
...
| setup-crumb.sh | script | contingency-keep | machine-bootstrap/rebuild script (references vault-check.sh); no runtime refs; VO-023 verification 2026-06-10: zero refs to any delete-listed script | keep (disaster-recovery path; VO-023 verify passed) | VO (B4) | — |
...
| hallucination-detection-protocol | protocol | contingency-keep → structural (VO-024) | referenced by spec v2-4 + architecture/02 + separate-version-history; CLAUDE.md Hallucination Detection § cites spec §4.8 (not this file directly); VO-024 comparison: authoritative expansion of §4.8 with unique operative depth (falsifiability examples, confidence-promotion rules, monthly procedure) | **keep** (VO-024 R13) | VO (B4) | — |
...
| com.crumb.qmd-index | plist | proven-active | loaded; produces qmd search index consumed by interactive vault search + session-report step (used 2026-06-10) | prop: keep (VO-023 #F5 — A3 extension, operator confirm at B4 pack approval) | VO | — |
...
| com.crumb.vault-rebuild | plist | structural-necessity | loaded; runs `~/quartz-vault/rebuild.sh` — produces the site A3-kept vault-web serves | prop: keep (VO-023 #F5 — A3 extension, operator confirm at B4 pack approval) | VO | — |
...
| claude-print-automation-patterns | solution | proven-active | refs=36, last=2026-04-04; subject (--print automation) partly sunset-tied but harness knowledge durable | keep (VO-024 R14 re-check passed) | VO (B3) | — |
...
| egpu-local-compute-evaluation | solution | contingency-keep | refs=2, last=2026-04-16; WATCH(30d) expired 2026-05; decision context (Tess v2 fine-tuning) decommissioned | prop: delete (VO-024 R11; **operator flag #F1** in B3 pack) | VO (B3) | — |
...
| haiku-soul-behavior-injection | solution | contingency-keep | refs=11; subject decommissioned but pattern generalizes (small-model behavioral ceiling → dedicated session prompts) | **keep** (VO-024 R12; write-only-from-ledger precedent) | VO (B3) | — |
...
| write-only-from-ledger | solution | proven-active | refs=15, last=2026-04-04; subject (FIF ledger) sunset-tied but pattern generalized | keep (VO-024 R15 re-check passed) | VO (B3) | — |
...
| anthropic-consolidation-hypothesis | doc | proven-active (light) | refs=4, last=2026-04-21; verified-claims analysis preceding the agentic-sunset decision | **keep** (VO-024 R1 — compound-provenance for the consolidation/sunset decision chain) | VO (B3) | — |
...
| liberation-surfaces-snapshot | doc | superseded (VO-024) | refs=3; frozen v1.1 four-surface snapshot — directive v2.1 dropped surface architecture; the surfaces themselves decommissioned | prop: delete (VO-024 R6; git provenance) | VO (B3) | — |
| mirror-config.yaml | data file | structural-necessity | consumed by mirror-sync.sh (hook chain, ran today) | keep | VO (B3) | — |
| network-kb-plan | doc | superseded (executed plan) | refs=1; all 9 vault-knowledge sources DONE 2026-03-06 (stated inside) | prop: delete (VO-024 R5; executed-spec precedent, provenance in git) | VO (B3) | — |
...
| security-kb-plan | doc | superseded (executed plan) | refs=1; batches 1–5 DONE + batch 6 dispatched (session-log:428) | prop: delete (VO-024 R5; durable guidance lives in cybersecurity-kb-capture + security-kb-sources) | VO (B3) | — |
...
| separate-version-history-archive | doc | structural-necessity (VO-024) | refs=2; functional version-history partition (v0.1–v1.9.1), cross-referenced from live history doc | **keep** (VO-024 R7) | VO (B3) | — |
| signals-archive-2026.jsonl | data file | superseded (dead producer) | refs=2, last=2026-03-01; FIF raw signal archive — producer decommissioned (storage-policy dead-producer rule); promoted signal notes live in Sources/signals/ (KB, kept) | prop: delete (VO-024 R16; AS concurrence — sunset-tied) | VO (B3) | — |
...
| system-architecture-diagram | doc | superseded (VO-024) | refs=6; 16-line redirect stub, `status: archived` — content absorbed into architecture/01 (DOH-005) | prop: delete (VO-024 R8; refs re-point to architecture/01) | VO (B3) | — |
...
| vault-intake-map | doc | superseded (VO-024 currency check) | refs=3; 5 of 8 documented intake paths decommissioned (FIF, Telegram digest, bridge, save command, research dispatch); live paths covered by spec §7 + capture-tiers | prop: delete (VO-024 R9; **operator flag #F2** — refresh declined, no named consumer) | VO (B3) | — |
...
| vault-restructure-analysis-20260220 | doc | superseded (VO-024) | refs=5; predecessor restructure analysis — superseded by v3 ADR + this project's spec/manifest | prop: delete (VO-024 R10; git provenance) | VO (B3) | — |
| vault-restructure-discussion-20260220 | doc | superseded (VO-024) | refs=5; discussion record for above | prop: delete with parent (VO-024 R10) | VO (B3) | — |
...
| docs/operator/explanation/ (3 of 4) | doc cluster | proven-active | last=2026-04-11; operator-facing Diátaxis layer | keep (exploded at VO-024) | VO (B3) | — |
| docs/operator/explanation/why-two-agents.md | doc | superseded (VO-024) | explains the Crumb+Tess two-agent architecture — decommissioned | prop: delete (VO-024; AS concurrence — sunset-tied) | VO (B3) | — |
...
| docs/operator/tutorials/first-crumb-session.md | doc | proven-active | current | keep (exploded at VO-024) | VO (B3) | — |
| docs/operator/tutorials/first-tess-interaction.md | doc | superseded (VO-024) | tutorial for decommissioned Tess interaction | prop: delete (VO-024; AS concurrence — sunset-tied) | VO (B3) | — |
| docs/operator/tutorials/mission-control-orientation.md | doc | proven-active (partial staleness) | dashboard kept per A3; dead panels described | keep — refresh at B3 (strip dead panels; VO-024 refresh list) | VO (B3) | — |
...
| _system/schemas/a2a/ (2) + capabilities/ (1) | schema cluster | superseded | Tess dispatch schemas (a2a envelopes, capability manifest); dispatch surface decommissioned; last=2026-03-22; no kept-skill consumers | prop: delete (AS concurrence — sunset-tied) | VO (B3) | — |
| _system/schemas/briefs/ (4 files) | schema cluster | superseded | brief schemas; dispatch surface dead BUT kept-skill `brief_schema:` frontmatter lines consume (A2 #3) | prop: delete **at B5** (exploded from cluster row; single-changeset coordination with frontmatter strips — VO-023 B5 pack) | VO (B5) | — |
