...
| id | description | state | depends_on | risk | domain | acceptance_criteria |
|---|---|---|---|---|---|---|
| TDM-010 | Commit + push every repo (vault + 7 openclaw repos) to clean working trees. **Runs AFTER TDM-014** so writers are stopped and the tree is stable; commit as the tess git author before logout. | **done** | TDM-014 | med | software | ✅ 2026-06-08 — vault committed `75c3d37f` + pushed to origin/main (clean tree, 0 uncommitted). Dead/no-remote repos (opportunity-scout 177-churn, book-scout, semuta) intentionally ride the rsync per M2 decision, not force-committed. |
| TDM-011 | Snapshot keychain item inventory to `~/migration-keychain-manifest.txt` | **done** | TDM-003 | low | software | ✅ `~/migration-keychain-manifest.txt` exists (13:55 live capture), contains all 15 Tier-A names + Tier-A′ items (gh, Claude Code, Safe Storage). Verified present this session. |
| TDM-012 | Capture running-services baseline (`launchctl list` filtered) to `~/migration-services-before.txt` | **done** | — | low | software | ✅ `~/migration-services-before.txt` exists (13:55 live capture), all 22 Crumb/hermes/openclaw agents with PID/status. Verified present this session. |
...
| TDM-014 | **Durably** freeze tess Crumb agents: `launchctl disable gui/501/<label>` (persistent override DB) + `bootout` each, then **log out of tess**. bootout alone is session-scoped — reversed by any tess login (observed 2026-06-08 14:42, see run-log). | **done (logout pending operator)** | TDM-012 | med | software | ✅ 2026-06-08 — all 22 labels `disable`d (gui/501 override DB, verified count=22) + booted out; `launchctl list` Crumb count = 0; no Crumb processes running. ⏳ Final sub-criterion "tess logged out" = operator action before P1. |
